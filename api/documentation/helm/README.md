# webresourcecatalogus chart

The Helm chart installs webresourcecatalogus and by default the following dependencies using subcharts:
...
```bash
helm repo add webresourcecatalogus https://raw.githubusercontent.com/ConductionNL/webresourcecatalogus/master/api/helm/
helm repo update
```
...
```bash
helm install my-webresourcecatalogus ConductionNL/webresourcecatalogus --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `webresourcecatalogus` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `webresourcecatalogus` |
